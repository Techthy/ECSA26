...
public class UncertaintyReactionsHelper {
    ...
    public static List<Uncertainty> createMissingUncertaintiesForReferencedComponents(
        ...
        // Is Set Manually is just done to keep the console output clean for now
        if (sourceUncertainty.isSetManually() && uncertaintyRepo.getHistory() != null
                && uncertaintyRepo.getHistory().getEntries() != null
                && !uncertaintyRepo.getHistory().getEntries().isEmpty()) {
            List<Uncertainty> historicalUncertainties = new ArrayList<>();
            uncertaintyRepo.getHistory().getEntries().stream()
                    .map(entry -> entry.getUncertainty())
                    .forEach(historicalUncertainties::add);
            List<Uncertainty> alreadyResolvedUncertainties = historicalUncertainties.stream()
                    .filter(u -> compareUncertainties(sourceUncertainty, u))
                    .toList();

            System.out.println("Found the following same uncertainties that were already resolved: ");
            for (Uncertainty u : alreadyResolvedUncertainties) {
                System.out.println(
                        "Uncertainty ID: " + u.getId() + " | Kind: " + u.getKind() + " | Nature: " + u.getNature() +
                                " | Reducability: " + u.getReducability() + " | Location: "
                                + u.getUncertaintyLocation().getSpecification());
            }

            if (uncertaintyRepo.getHistory().isAutomatedResolution() && !alreadyResolvedUncertainties.isEmpty()) {
                uncertaintyRepo.getUncertainties().remove(sourceUncertainty);
                System.out
                        .println(
                                "Source uncertainty removed as it was already resolved in history and automated resolution is enabled.");
