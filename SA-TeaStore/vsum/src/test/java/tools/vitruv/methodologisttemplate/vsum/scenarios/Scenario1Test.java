...
public class Scenario1Test {
    ...
    /**
     * Scenario 1: Add Uncertainty to ADR Alternatives that affect C2 Containers
     ...
     * The Architecture Decision Record has an Issue that that has one ADR
     ...
     */
    @Test
    void scenario1(@TempDir Path tempDir) {
        ...
        checkC2ContainerUncertaintyPropagation(uncertaintyRepo, "Cache");
        checkC2ContainerUncertaintyPropagation(uncertaintyRepo, "PersistenceService");
        ...
        checkPCMComponentUncertaintyPropagation(uncertaintyRepo, "PersistenceService");

        // Check total number of uncertainties matches
        // There is initially 1 uncertainties defined in the uncertainty model
        // + 7 uncertainties propagated (1 ADIssue, 3 C2 Containers, 2 PCM Components, 1
        // Ticket)
        Assertions.assertEquals(8, uncertaintyRepo.getUncertainties().size());
    ...
    private void registerRootObjects(VirtualModel virtualModel, Path filePath) {
        ...
        URIConverter.URI_MAP.put(
                ...
                URI.createFileURI("./src/test/resources/teastore_scenario1.architecturedecisionrecord"));
        ...
        Resource pcmResource = TestUtil.loadResource(resourceSet, "./src/test/resources/teastore_scenario1.repository");
    ...
    private void checkADRIssueUncertaintyPropagation(UncertaintyAnnotationRepository uncertaintyRepo) {
        ...
        adIssueUncertainties.stream().filter(
                u -> ((ADIssue) u.getUncertaintyLocation().getReferencedComponents().get(0)).getId().equals("ADR-101"))
                .findAny()
                .orElseThrow(() -> new AssertionError("Expected Uncertainty referencing ADIssue ADR-101 not found."));
        System.out.println("	• ADIssue PASSED: Uncertainty referencing ADIssue ADR-101 found.");
    }
